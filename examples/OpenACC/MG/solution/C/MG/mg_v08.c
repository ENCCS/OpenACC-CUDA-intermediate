...
c-------------------------------------------------------------------*/
    ...
    // First we need some offsets so we know where each array starts
    int summ321[MAXLEVEL+1];
    summ321[0] = 0;
    for (l = 1; l <= lt; l++) {
      summ321[l] = summ321[l-1] + m3[l]*m2[l]*m1[l];
    }

    u = (double **)malloc((lt+1)*sizeof(double *));
    double *udata = (double *)malloc(summ321[lt]*sizeof(double));

    v = (double *)malloc(m3[lt]*m2[lt]*m1[lt]*sizeof(double));

    r = (double **)malloc((lt+1)*sizeof(double *));
    double *rdata = (double *)malloc(summ321[lt]*sizeof(double));

    // The pointers to the data for the different levels in host memory
    for (l = lt; l >=1; l--) {
      u[l] = &(udata[summ321[l-1]]);
      r[l] = &(rdata[summ321[l-1]]);
    }

#pragma acc data create(u[0:lt+1],udata[0:summ321[lt]]) \
  create(v[0:m3[lt]*m2[lt]*m1[lt]]) \
  create(r[0:lt+1],rdata[0:summ321[lt]]) \
  copyin(a[0:4],c[0:4])
    {

      // The pointers to the data for the different levels in device memory
      // We use the same offsets here
...
#endif /*_OPENACC */
#else /* USE_CUDA */
#pragma acc parallel loop private(u1,u2) vector_length(NTHREADS) \
...
#pragma acc parallel loop vector_length(NTHREADS) private(x1,y1)	\
  pcopyout(s[0:m1j*m2j*m3j])			\
  pcopyin(r[0:m1k*m2k*m3k])
...
#pragma acc parallel loop vector_length(NTHREADS) \
  private(tmp,i123) reduction(+:p_s) reduction(max:p_a) pcopyin(r[0:n3*n2*n1])
    for (i3 = 1; i3 < n3-1; i3++) {
	for (i2 = 1; i2 < n2-1; i2++) {
            for (i1 = 1; i1 < n1-1; i1++) {
	      i123 = i1 + n1*(i2 + n2*i3);
...
c-------------------------------------------------------------------*/
    ...
    /* axis = 1 */
#pragma acc parallel loop gang vector_length(NTHREADS) pcopy(u[0:n3*n2*n1])
    for ( i3 = 1; i3 < n3-1; i3++) {
#pragma acc loop
#pragma novector
	for ( i2 = 1; i2 < n2-1; i2++) {
    ...
    /* axis = 2 */
#pragma acc parallel loop vector_length(NTHREADS) pcopy(u[0:n3*n2*n1])
    for ( i3 = 1; i3 < n3-1; i3++) {
#pragma acc loop
#pragma novector
	for ( i1 = 0; i1 < n1; i1++) {
...
#pragma acc parallel pcopy(z[0:n3*n2*n1]) copyin(jg)
{
#pragma acc loop
    for (i = MM-1; i >= m0; i--) {
...
#pragma acc data pcopyin(z[0:n3*n2*n1])
    {
