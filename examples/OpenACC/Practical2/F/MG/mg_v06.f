...
c          H. Jin
c
c OpenMP acceleration: A. Hart (ahart@cray.com)
c
c---------------------------------------------------------------------
...
!$acc data create(u,v,r) copyin(a,c)
...
!$acc parallel loop vector_length(NTHREADS)
!$acc& private(u1,u2) present(u,r,v,a)
      do i3=2,n3-1
         do i2=2,n2-1
            do i1=1,n1
...
!$omp do
!$acc parallel loop vector_length(NTHREADS) present(z,u)
         do  i3=d3,mm3-1
            do  i2=d2,mm2-1
               do  i1=d1,mm1-1
...
!$acc parallel loop vector_length(NTHREADS)
!$acc& private(a) reduction(+:s) reduction(max:rnmu) present(r)
      do  i3=2,n3-1
         do  i2=2,n2-1
            do  i1=2,n1-1
               s=s+r(i1,i2,i3)**2
...
!$omp parallel default(shared) private(i1,i2,i3)
!$omp do
!$acc parallel loop vector_length(NTHREADS) present(u)
      do  i3=2,n3-1
         do  i2=2,n2-1
...
c The array z is used in two ways. First it is just scratch space.
c Then at the end it is is populated with the final data.

c With OpenACC, porting the random number generator and the bubblesort
c is best avoided. So we will generate the jg array on the host, using
c the host version of z as the temporary array. For this reason, we
c explicitly zero z below rather than using the zero3 subroutine.

c Array jg will be copied to the GPU and z populated there. For this
c reason, the zeroing of z is done using zero3() rather than with an
c explicit loopnest.
...
c From now on, with OpenACC all the references to z are the GPU version
      call zero3(z,n1,n2,n3)

!$acc parallel vector_length(NTHREADS) present(z) copyin(jg)
!$acc loop
      do  i=mm,mm0,-1
