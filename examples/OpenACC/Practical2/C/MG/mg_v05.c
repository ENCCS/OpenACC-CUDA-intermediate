...
c-------------------------------------------------------------------*/
    ...
    /* axis = 1 */
#pragma omp for
#pragma acc parallel loop gang vector_length(NTHREADS) copy(u[0:n3*n2*n1])
    for ( i3 = 1; i3 < n3-1; i3++) {
#pragma acc loop
#pragma novector
	for ( i2 = 1; i2 < n2-1; i2++) {
    ...
    /* axis = 2 */
#pragma omp for
#pragma acc parallel loop vector_length(NTHREADS) copy(u[0:n3*n2*n1])
    for ( i3 = 1; i3 < n3-1; i3++) {
#pragma acc loop
#pragma novector
	for ( i1 = 0; i1 < n1; i1++) {
...
#pragma acc data copyin(z[0:n3*n2*n1])
    {
